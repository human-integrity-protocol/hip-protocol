...
### REVISED DRAFT | 2026-02-28 | Pre-inscription
...
**Vouching:** The act by which a credentialed human attests that a new
...
**Vouching Record (Tier 3 credentials only):** The credential identifier
...
### Tier 2 — Device-Attested Issuance

Tier 2 pathways use hardware-level biometric liveness attestation via
...
liveness (a modern smartphone with a manufacturer-supported biometric
attestation API). The application requests a liveness attestation from
the device's hardware security module. Upon receiving a valid,
hardware-signed attestation that a genuine biometric check was performed
on a genuine, manufacturer-verified device, the credential is issued.

**Initial IssuanceWeight (Tier 2):** 150

**Initial Trust Index (Tier 2):** 150 (IssuanceWeight: 150 + BehavioralScore: 0)
...
required at issuance beyond what the Tier 2 process entails.

**Device binding:** A Tier 2 credential is initially bound to the device
on which it was issued. Device migration is handled through the portability
mechanics defined in this specification.

**Pseudonymity:** Tier 2 establishes device-level liveness, not identity.
The ledger record confirms that Tier 2 issuance was completed; it does not
record which device, which biometric, or which individual was involved.

---

### Tier 3 — Vouched Issuance

Tier 3 pathways use peer vouching: one or more existing credentialed
humans attest that a new credential applicant is a living human known
to them. This is the most globally accessible pathway and operates without
external infrastructure dependencies.

**Issuance process:** The credential applicant is vouched for by a vouching
...
vouching transaction. Upon receipt of at least one valid vouch from an
eligible vouching credential, the applicant's credential is issued.

**Initial IssuanceWeight (Tier 3):** 50

**Initial Trust Index (Tier 3):** 50 (IssuanceWeight: 50 + BehavioralScore: 0)

**Vouching eligibility requirements:**
A credential holder is eligible to vouch if:
- Their credential is in Active lifecycle state
...
**Single-vouch sufficiency:** One valid vouch from an eligible credential
is sufficient to trigger issuance. Additional vouches may be submitted
and are recorded in the credential's vouching record, providing additional
context to verifiers.
...
the structural mechanism by which the protocol makes Tier 3 meaningful
...
+ BehavioralScore). A Tier 3 credential with IssuanceWeight of 50 may
accumulate a BehavioralScore of 950, reaching the same TI ceiling as
...
IssuanceWeight of 50 and a long, clean behavioral history may accumulate
a BehavioralScore of 950, reaching the TI ceiling of 1000 — the same
...
structural advantage. A credential that was issued through peer vouching
...
Awarded to the vouching credential holder when a Tier 3 credential
...
Tier 3 vouching carries meaningful weight.
...
Example: A Tier 2 credential with IssuanceWeight of 150 issued through
a pathway that subsequently enters Watch state has its IssuanceWeight
updated to 113 (150 × 0.75, rounded to nearest integer). If the pathway
subsequently exits Watch state and returns to Active state (through
remediation), the IssuanceWeight is restored to the full 150.
...
A Tier 3 credential begins at TI 50. It reaches the floor after
approximately 3-4 contemporaneous attestations of any content —
...
A Tier 2 credential begins at TI 150. It clears the floor at issuance.
...
- "Verified via Device Biometric" (Tier 2)
- "Verified via Peer Vouching" (Tier 3)
...
**Vouching eligibility floor:** A credential must have a current Trust
Index of at least **200** to be eligible to issue vouches. This floor
ensures that a credential must have demonstrated meaningful standing
before it can extend that standing to others.
...
- If a Tier 3 vouch was involved in the fraudulent issuance, the
...
**Resolved — Initial tier weights (400 / 150 / 50):**
Tier 1 at 400: durable, externally cross-referenced verification.
Tier 2 revised to 150 (from 200): device biometrics are strong today
but the human-accomplice attack vector is present and real. PHI monitoring
handles future degradation automatically; initial weight reflects honest
current assurance differential. Tier 3 at 50: accessible floor.
Ratio approximately 8:3:1. Confirmed design.
...
*HP-SPEC-v1: HUMAN-PROOF Credential Mechanics Specification — REVISED DRAFT.*
*All open items resolved, including CDI-2 (algorithmic flag treatment) and
CDI-3 (anonymous reporting).*
...
*Revision incorporates: algorithmic flag monitoring-not-suspension model,
Suspension Impact Statement requirement, anonymous reporting with elevated
evidence threshold, "Adjudication" to "Determination" header fix.*
