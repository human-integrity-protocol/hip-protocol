...
## CRYPTO-SPEC-v1.1: Cryptographic Primitives Specification
### v1.1 REVISION | 2026-03-28 | Post-Quantum Posture Update
...
- **INT-SPEC-v1: Integration Specification** — which defines the
  ...
  specification
- **HP-SPEC-v1.2: HUMAN-PROOF Credential Mechanics Specification** —
...
Ed25519 is vulnerable to Shor's algorithm on a sufficiently powerful
quantum computer. A cryptographically relevant quantum computer (CRQC)
could derive a private key from a public key, enabling forgery of
credential signatures and Guardian Key operations. The Guardian Public
Key is intentionally public — required for open verification — and
constitutes the input Shor's algorithm would require.

Current consensus places CRQCs capable of attacking 256-bit elliptic
curves at a minimum of one to two decades away. This is not a reason
to treat the threat as hypothetical; it is a reason to define the
migration path now.

**Target algorithm:** NIST finalized ML-DSA (CRYSTALS-Dilithium) as
its primary post-quantum signature standard in 2024. ML-DSA is the
designated replacement for Ed25519 in HIP when post-quantum transition
becomes necessary.

**Size implications:** ML-DSA-44 (the smallest parameter set) produces
public keys of 1,312 bytes and signatures of 2,420 bytes, compared to
Ed25519's 32-byte public keys and 64-byte signatures. This substantially
exceeds the current Proof Bundle size budget and the Proof Anchor
256-byte budget. When transition occurs:

- The Proof Anchor budget will require revision in WF-SPEC (a design
  goal update, not a structural change to on-chain mechanics)
- Hybrid schemes (Ed25519 + ML-DSA in parallel) may be specified for
  the transition period, providing immediate post-quantum resistance
  while maintaining classical verifiability for existing records
- CRYPTO-SPEC-v2 will specify the exact parameter sets, wire format
  identifiers, and size budget adjustments

**Guardian Key migration:** The Guardian Key rotation and credential
re-attestation process required for this transition is defined in
HP-SPEC-v1.2 (Post-Quantum Posture section). The migration path uses
the existing credential upgrade and recovery infrastructure. No
architectural change to the credential lifecycle model is required.

**SHA-256 posture:** SHA-256 used for content hashing, Bundle-to-Anchor
hashing, and credential identifier derivation is not meaningfully
threatened. Grover's algorithm reduces effective security from 256 to
128 bits — still well above any foreseeable attack threshold. SHA-256
is not scheduled for replacement on any quantum-driven timeline.

**HMAC-SHA-256 posture:** HMAC-SHA-256 inherits SHA-256's posture.
Not scheduled for replacement.

This transition is planned, not speculative. The algorithm transition
model above accommodates it. CRYPTO-SPEC-v2 will be issued when
ML-DSA deployment becomes practical.
...
- **HP-SPEC-v1.2**, which defines the credential key model. Ed25519
  ...
  HP-SPEC-v1.2's requirement that credential identifiers not
  contain identity information. HP-SPEC-v1.2 also defines the
  post-quantum migration path (Guardian Key rotation, re-attestation
  window) that CRYPTO-SPEC-v1.1's Post-Quantum Consideration section
  references.
...
- Post-quantum transition to ML-DSA (CRYSTALS-Dilithium): migration
  path defined in HP-SPEC-v1.2; CRYPTO-SPEC-v2 will specify parameter
  sets, wire format identifiers, and size budget adjustments when
  deployment becomes practical
- ECDSA-P256 dual-support for regulatory environments (demand-driven;
...
*CRYPTO-SPEC-v1.1: Cryptographic Primitives Specification.*
*v1.1 updates: Post-Quantum Consideration expanded with concrete ML-DSA migration plan,*
*size implications, hybrid scheme guidance, and cross-reference to HP-SPEC-v1.2.*
*HP-SPEC reference updated to v1.2. Post-quantum moved from speculative to planned.*
*SHA-256 for all hashing. Ed25519 for digital signatures (ML-DSA designated successor).*
*HMAC-SHA-256 for privacy mechanisms. Conservative selection, universal availability,*
*agility through versioning.*
